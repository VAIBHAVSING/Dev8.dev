...
async function main() {
  ...
  const templates = [
    {
      name: 'node',
      displayName: 'Node.js',
      description: 'JavaScript and TypeScript development with Node.js LTS, npm, pnpm, and common tools',
    ...
    },
    {
      name: 'python',
      displayName: 'Python',
      description: 'Python 3.11 development environment with pip, poetry, and data science libraries',
    ...
    },
    {
      name: 'golang',
      displayName: 'Go',
      description: 'Go 1.21+ development environment with standard toolchain and popular tools',
    ...
    },
    {
      ...
      description: 'Python data science stack with Jupyter, pandas, numpy, and ML libraries',
  ...
  for (const template of templates) {
    ...
    console.log(`  ✅ ${result.displayName} (${result.isPopular ? 'Popular' : 'Standard'})`);
  }

  console.log('\n📊 Seed summary:');
  const totalTemplates = await prisma.template.count();
  const popularTemplates = await prisma.template.count({ where: { isPopular: true } });
